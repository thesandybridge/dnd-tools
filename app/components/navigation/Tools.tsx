...
export default function ToolsNav() {
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      <h1 className="text-2xl font-bold font-cinzel">Adventurer's Toolkit</h1>
        ...
        {routes.map(route => {
          ...
          return (
            <Link key={route.path} href={`/tools/${route.path}`}>
              <GlassPanel
                coronaHover
                className={`h-full cursor-pointer transition-all hover:scale-[1.02] ${isActive ? 'border-primary bg-primary/5' : ''}`}
              >
                <div className="flex flex-col items-center text-center gap-3 p-6">
                  ...
                  <div>
                    <p className="font-cinzel font-semibold text-foreground">{route.label}</p>
