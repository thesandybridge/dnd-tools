...
function InlineEdit({ marker, onDone }: { marker: Marker; onDone: () => void }) {
...
export function MarkerInfoCard({ marker, screenPosition, containerSize, onDismiss }: MarkerInfoCardProps) {
  ...
  // Position card above the marker, centered horizontally
  // Clamp so it stays within the container bounds
  ...
  // If card would go above viewport, show below marker instead
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          {editing ? (
            <div className="flex-1 min-w-0">
              <InlineEdit marker={marker} onDone={() => setEditing(false)} />
            </div>
          ) : (
